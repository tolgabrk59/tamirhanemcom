...
export default function Sidebar() {
  ...
  const { appointmentBadge, notificationBadge, isConnected } = useRealtimeBadge(jwt);
  ...
  const staticNavLinks = [
    ...
    { href: '/oto-yikama', label: 'Oto Yıkama', icon: OtoYikamaIcon },
    { href: '/kampanyalar', label: 'Kampanyalar', icon: TagIcon },
  ];

  const authNavLinks = [
    { href: '/randevularim', label: 'Randevularım', icon: CalendarIcon },
    { href: '/bildirimler', label: 'Bildirimler', icon: BellIcon },
    { href: '/cuzdan', label: 'Cüzdan', icon: CalculatorIcon },
    { href: '/profil', label: 'Profil', icon: UserIcon },
  ];

  const navLinks = isLoggedIn
    ? [...staticNavLinks, ...authNavLinks]
    : staticNavLinks;
      ...
      <div className="flex flex-col h-full">
            ...
            return (
              ...
              >
                <span className="flex-shrink-0 relative">
                  <Icon />
                  {!showLabels && link.href === '/bildirimler' && notificationBadge > 0 && (
                    <span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full" />
                  )}
                  {!showLabels && link.href === '/randevularim' && appointmentBadge > 0 && (
                    <span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full" />
                  )}
                </span>
                {showLabels && (
                  <>
                    ...
                    {link.href === '/randevularim' && appointmentBadge > 0 && (
                    ...
                    {link.href === '/bildirimler' && notificationBadge > 0 && (
                      <span className="ml-auto bg-red-500 text-white text-xs font-bold rounded-full min-w-[20px] h-5 flex items-center justify-center px-1">
        ...
        {showLabels && (
          <div className="p-4 border-t border-[#454545]/20 space-y-2">
            ...
            {showLoginSpotlight && alwaysExpanded && !isLoggedIn && (
            ...
            {isLoggedIn ? (
              <>
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full bg-white/10 text-[#454545] px-4 py-2.5 rounded-lg font-semibold hover:bg-white/20 transition-colors border border-[#454545]/20 font-[family-name:var(--font-jakarta)]"
                >
                  <LoginIcon />
                  Çıkış Yap
                </button>
        ...
        {!showLabels && (
          <div className="p-2 pb-4 border-t border-[#454545]/20 space-y-1">
            {isLoggedIn ? (
              <>
                <Link
                  href="/profil"
        ...
        <div className="flex items-center justify-between h-14 px-4">
          ...
          <div className="flex items-center gap-1">
            {isLoggedIn && (
              <Link
                href="/bildirimler"
                className="relative p-3 rounded-lg hover:bg-primary-500 transition-colors"
                title="Bildirimler"
              >
                <BellIcon />
                {notificationBadge > 0 && (
                ...
                {isConnected && (
                  <span className="absolute bottom-1.5 right-1.5 w-1.5 h-1.5 bg-green-400 rounded-full" title="Canlı bağlantı aktif" />
                )}
              </Link>
            )}
            <button
      ...
      <nav className="hidden lg:flex fixed bottom-0 left-16 right-0 h-14 bg-primary-500 z-40 items-center justify-between px-2 2xl:px-6 overflow-visible">
        ...
        <div className="flex items-center gap-0.5 2xl:gap-1 flex-1 min-w-0 overflow-visible">
          {navLinks.map((link) => {
            const Icon = link.icon;
            const badge =
              link.href === '/randevularim' ? appointmentBadge :
              link.href === '/bildirimler' ? notificationBadge : 0;
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`relative flex items-center gap-1 2xl:gap-2 px-1.5 lg:px-2 2xl:px-4 py-2 rounded-lg transition-all flex-shrink
                  ${isActiveLink(link.href)
                    ? 'bg-white/20 text-[#454545]'
                    : 'text-[#454545] hover:bg-white/10'
                  }`}
              >
                ...
                {badge > 0 && (
                  <span className="bg-red-500 text-white text-[10px] font-bold rounded-full min-w-[16px] h-4 flex items-center justify-center px-0.5 leading-none">
                    {badge > 99 ? '99+' : badge}
                  </span>
                )}
              </Link>
        ...
        <div className="flex items-center gap-1 2xl:gap-2 flex-shrink-0 ml-2">
          {isLoggedIn ? (
            <>
              ...
              >
                <UserIcon />
                <span className="hidden xl:inline">Profilim</span>
              </Link>
              <button
                onClick={() => {
                  localStorage.removeItem('tamirhanem_jwt');
