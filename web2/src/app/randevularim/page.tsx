...
const STEPPER_STEPS = [
    ...
    { label: 'Hizmet', statuses: ['Araç Teslim Alındı', 'İşlem Devam Ediyor', 'Müşteri Onayı Bekleniyor'] },
                        ...
                        <div className="flex flex-col items-center">
                            ...
                            <span
                                className={`text-[9px] mt-1 font-medium whitespace-nowrap ${
                                    isCompleted || isActive ? 'text-orange-400' : 'text-gray-600'
                                }`}
                            >
                                {step.label}
                            </span>
            ...
            <div className="space-y-2 mb-4">
                ...
                {apt.attributes.scheduledDate && (
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                        ...
                        <span>Oluşturulma: {formatDate(apt.attributes.createdAt)}</span>
                    </div>
                )}
            </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
            <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-md p-6">
                ...
                <textarea
                    ...
                    rows={3}
                    className="w-full bg-gray-800 border border-gray-700 text-white text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-orange-500 resize-none placeholder-gray-500 mb-4 transition-colors"
                />
    ...
    const fetchAppointments = useCallback(async (token: string) => {
        ...
        try {
            const res = await fetch(
                '/api/appointments?populate=vehicle,service,services&sort=createdAt:desc',
    ...
    if (!jwt) {
        return (
            <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center text-center px-4">
                ...
                <p className="text-gray-400 text-sm mb-6">Randevularınızı görüntülemek için hesabınıza giriş yapın.</p>
                <Link
                    href="/giris"
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <p className="text-white font-semibold mb-1">
                            {activeTab === 'aktif' ? 'Aktif randevunuz yok' : 'Geçmiş randevunuz yok'}
                        </p>
                        <p className="text-gray-500 text-sm mb-6">
                            {activeTab === 'aktif'
                                ? 'Şu anda aktif bir randevunuz bulunmuyor.'
                                : 'Tamamlanmış veya iptal edilmiş randevunuz yok.'}
                        </p>
                        {activeTab === 'aktif' && (
                            <Link
                                href="/"
