...
function ServiceResultsContent() {
    ...
    // Nearby mode — from GPS
    const nearbyLat = searchParams.get('lat');
    const nearbyLng = searchParams.get('lng');
    const isNearbyMode = searchParams.get('nearby') === 'true' && !!nearbyLat && !!nearbyLng;
    ...
    useEffect(() => {
        const fetchServices = async () => {
            setLoading(true);
            try {
                if (isNearbyMode && nearbyLat && nearbyLng) {
                    const params = new URLSearchParams({ lat: nearbyLat, lng: nearbyLng });
                    const response = await fetch(`/api/services/nearest?${params.toString()}`);
        ...
        fetchServices();
    }, [isNearbyMode, nearbyLat, nearbyLng, selectedCity, selectedDistrict, selectedBrand, selectedModel, selectedCategory, selectedFuelType]);
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            {isNearbyMode && (
                <div className="bg-orange-50 border-b border-orange-200">
                    <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2">
                            <svg className="w-4 h-4 text-orange-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" />
                            </svg>
                            <span className="text-sm text-orange-700 font-medium">
                                Konumunuza yakın servisler gösteriliyor
                            </span>
                        </div>
                        <button
                            onClick={() => router.push('/servisler/sonuclar')}
                            className="text-xs text-orange-600 hover:text-orange-800 underline whitespace-nowrap"
                        >
                            Tümünü Göster
                        </button>
                    </div>
