...
export function searchEncyclopedia(query: string): { system: EncyclopediaSystem; component: EncyclopediaComponent; subsystem?: EncyclopediaSubsystem; score: number }[] {
    const results: { system: EncyclopediaSystem; component: EncyclopediaComponent; subsystem?: EncyclopediaSubsystem; score: number }[] = [];
    const words = query.toLowerCase().split(/\s+/).filter(w => w.length >= 2);
    if (words.length === 0) return results;

    function matchScore(component: EncyclopediaComponent, systemName: string): number {
        const nameLower = component.name.toLowerCase();
        const descLower = component.description.toLowerCase();
        const funcLower = (component.function || '').toLowerCase();
        const symptomsLower = component.symptoms.map(s => s.toLowerCase());
        const repairLower = component.repairAdvice.map(r => r.toLowerCase());
        const allText = [nameLower, descLower, funcLower, systemName.toLowerCase(), ...symptomsLower, ...repairLower].join(' ');
        ...
        for (const word of words) {
            if (allText.includes(word)) {
                matched++;
                // Ağırlıklı puanlama: isim > belirti > açıklama > diğer
                if (nameLower.includes(word)) score += 10;
                if (symptomsLower.some(s => s.includes(word))) score += 7;
                if (descLower.includes(word)) score += 4;
                if (funcLower.includes(word)) score += 3;
                if (repairLower.some(r => r.includes(word))) score += 2;
                if (systemName.toLowerCase().includes(word)) score += 1;
            }
        }
        // Tüm kelimeler eşleşmediyse sıfır döndür
        if (matched < Math.ceil(words.length * 0.5)) return 0;
    ...
    encyclopediaSystems.forEach(system => {
            ...
            system.subsystems.forEach(subsystem => {
                if (subsystem) {
                    subsystem.components.forEach(component => {
                        const score = matchScore(component, system.name);
                        if (score > 0) {
                            results.push({ system, component, subsystem, score });
                        }
                    });
                }
            });
        ...
        system.components.forEach(component => {
            const score = matchScore(component, system.name);
            if (score > 0) {
                results.push({ system, component, score });
            }
        });
    });
